"use client"
import AnimatedSection from "@/components/ui/AnimatedSection"
import GitHubContributions from "@/components/projects/GitHubContributions"

const GithubIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/>
  </svg>
)

const EmailIcon = () => (
  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
    <rect x="2" y="4" width="20" height="16" rx="2"/>
    <path d="m2 7 10 7 10-7"/>
  </svg>
)

const XIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.259 5.631 5.905-5.631zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
  </svg>
)

const links = [
  { icon: <GithubIcon />, label: "GitHub", href: "https://github.com/OsamaFed" },
  { icon: <XIcon />, label: "X", href: "https://x.com/osamafed?s=21" },
  { icon: <EmailIcon />, label: "osama.mohammed.work1@gmail.com", href: "mailto:osama.mohammed.work1@gmail.com" },
]

export default function Contact() {
  return (
    <section id="contact" className="px-6 py-12">
      <div className="max-w-2xl mx-auto">

        <AnimatedSection>
          <div className="mb-10">
            <h2 className="text-2xl font-semibold tracking-wider mb-6"
              style={{ fontFamily: "var(--font-mono, monospace)", color: "var(--theme-primary)" }}>
              Stats
            </h2>
            <GitHubContributions username="OsamaFed" />
          </div>
        </AnimatedSection>

        <AnimatedSection delay={0.1}>
          <h2 className="text-2xl font-semibold tracking-wider mb-2"
            style={{ fontFamily: "var(--font-mono, monospace)", color: "var(--theme-primary)" }}>
            get in touch.
          </h2>
          <p className="text-sm mb-8"
            style={{ fontFamily: "var(--font-mono, monospace)", color: "var(--theme-muted)" }}>
            open to work — feel free to reach out.
          </p>

          <div className="flex flex-col gap-4">
            {links.map((link) => (
              <a
                key={link.label}
                href={link.href}
                target={link.href.startsWith("mailto") ? undefined : "_blank"}
                rel="noreferrer"
                className="flex items-center gap-3 transition-all duration-200 w-fit"
                style={{ color: "var(--theme-muted)" }}
                onMouseEnter={e => (e.currentTarget.style.color = "var(--theme-primary)")}
                onMouseLeave={e => (e.currentTarget.style.color = "var(--theme-muted)")}
              >
                {link.icon}
                <span className="text-sm" style={{ fontFamily: "var(--font-mono, monospace)" }}>
                  {link.label}
                </span>
              </a>
            ))}
          </div>
        </AnimatedSection>

      </div>
    </section>
  )
}
